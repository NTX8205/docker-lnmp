...
> 目前 docker 使用者預設是 **root**，在使用 composer 生成的檔案權限為 root 需要注意
...
預設為關閉，需要使用請將 `docker-compose.yml` 中的註解取消。
